import { d as defineEventHandler, r as requireSession, b as readValidatedBody, c as createError, p as progressCollection, T as TIERS } from '../../nitro/nitro.mjs';
import { z } from 'zod';
import 'mongodb';
import 'better-auth';
...
}).refine((b) => Object.keys(b).length > 0, { message: "Corps de requ\xEAte vide" });
const progress_patch = defineEventHandler(async (event) => {
  var _a, _b, _c, _d;
  const session = await requireSession(event);
  const body = await readValidatedBody(event, bodySchema.parse);
  const unknownSlugs = Object.keys((_a = body.collection) != null ? _a : {}).filter((slug) => !KNOWN_SLUGS.has(slug));
  if (unknownSlugs.length) {
    throw createError({ statusCode: 400, statusMessage: "Droid inconnu dans la collection." });
  }
  const unknownItems = Object.keys((_b = body.shopLevels) != null ? _b : {}).filter((id) => !KNOWN_ITEMS.has(id));
  if (unknownItems.length) {
    throw createError({ statusCode: 400, statusMessage: "Article de boutique inconnu." });
  }
  const set = { updatedAt: /* @__PURE__ */ new Date() };
  for (const [slug, entry] of Object.entries((_c = body.collection) != null ? _c : {})) {
    set[`collection.${slug}`] = entry;
  }
  for (const [itemId, level] of Object.entries((_d = body.shopLevels) != null ? _d : {})) {
    set[`shopLevels.${itemId}`] = level;
  }
  for (const key of ["rebirth", "superRebirth", "cycle", "novaCrystals"]) {
...
export { progress_patch as default };
//# sourceMappingURL=progress.patch.mjs.map
