  ...
  >
    ...
    <section class="panel flex flex-col gap-5 p-5 md:flex-row">
      <div class="flex shrink-0 flex-col items-center gap-2">
        ...
        <p
          v-if="focusStats?.imageIsFallback"
          class="max-w-40 text-center text-xs text-tier-galactic"
        >
      ...
      <div class="min-w-0 flex-1">
        ...
        <p
          v-if="droid.perk"
          class="mt-2 rounded-lg bg-panel-raised px-3 py-2 text-sm"
        >
        ...
        >
          ...
          >
            <DroidImage
              ...
              :tier="tier"
              size="sm"
              :dimmed="!entry.tiers.includes(tier)"
            />
          </button>
        </div>

        <p class="mt-1 text-xs text-ink-muted">
          {{ $t('droid.flawlessHint') }}
        </p>

        <!-- Revenu en pourcentage pour les Emblématiques : pas de tableau par palier. -->
        <p
          v-if="droid.percentIncome"
          class="mt-4 text-lg text-iconic"
        >
    ...
    >
      ...
      <p class="px-4 py-3 text-xs text-ink-muted">
        {{ $t('droid.galacticNoData') }}
      </p>
    </section>

    <section
      v-if="usedInRebirths.length"
      class="panel p-4"
    >
      <h2 class="mb-2 font-semibold">
        {{ $t('droid.usedInRebirths') }}
      </h2>
