 ...
 * la mantisse : c'est lui qui met la virgule française (« 1,4B »). Le suffixe est collé au
 * nombre, sans espace, comme dans le jeu.
 *
 * Une décimale au plus : « 1,4M » est utile, « 340,2M » ne l'est pas.
 */
const SCALE = [
...
export function formatNumber(value: number | null | undefined, locale = 'fr'): string {
  ...
  return `${mantisse(locale).format(value / palier.seuil)}${palier.suffixe}`
}
